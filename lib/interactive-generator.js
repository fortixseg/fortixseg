...
export async function generateInteractiveCourseFromPdf({ bytes, originalName, storedUrl, storedPathname, options = {} }) {
  ...
  const course = {
    ...
    review: {
      required: true,
      status: "pending",
      notes: ["Revisao tecnica obrigatoria antes de publicar o treinamento."]
    },
...
function detectTemplate(text) {
  ...
  for (const rule of DETECTION_RULES) {
    const score = rule.keywords.reduce((total, keyword) => total + (haystack.includes(normalizeForSearch(keyword)) ? 1 : 0), 0);
...
function buildModules(templateSource, pages, pdfUrl) {
  const contentModules = templateSource.modules.map((module, index) => ({
    id: `module-${index + 1}`,
    title: module.title,
    topics: module.topics,
    lessons: []
  }));
  const targetModules = contentModules.filter((module) => !/avaliacao/i.test(normalizeForSearch(module.title)));
  const sourcePages = pages.length ? pages : buildFallbackPages(templateSource);

  sourcePages.forEach((page, index) => {
    const module = pickModule(targetModules, page.text, index);
    const lessonTitle = pickLessonTitle(module, page.text, index);
    module.lessons.push(buildLesson({ page, module, title: lessonTitle, pdfUrl, lessonIndex: module.lessons.length + 1, templateKey: templateSource.key }));
  });

  contentModules.forEach((module, moduleIndex) => {
    if (/avaliacao/i.test(normalizeForSearch(module.title))) return;
    const existingTitles = new Set((module.lessons || []).map((lesson) => normalizeForSearch(lesson.title)));
    const targetTopics = (module.topics || []).slice(0, 4);
    targetTopics.forEach((topic, topicIndex) => {
      if (module.lessons.length >= 4 || existingTitles.has(normalizeForSearch(topic))) return;
      const referencePage = pickReferencePageForTopic(sourcePages, topic) || module.lessons[0] || { pageNumber: moduleIndex + 1, text: "" };
      const page = {
        pageNumber: referencePage.sourcePage || referencePage.pageNumber || moduleIndex + 1,
        text: referencePage.text || referencePage.extractedText || `${topic}. Conteudo gerado pelo modelo ${templateSource.label} para revisao tecnica.`
      };
      module.lessons.push(buildLesson({ page, module, title: topic, pdfUrl, lessonIndex: topicIndex + 1, templateKey: templateSource.key }));
...
function pickReferencePageForTopic(pages, topic) {
  const normalizedTopic = normalizeForSearch(topic);
  return (pages || []).find((page) => normalizeForSearch(page.text || "").includes(normalizedTopic)) || (pages || [])[0] || null;
}
...
function buildLesson({ page, module, title, pdfUrl, lessonIndex, templateKey }) {
  ...
  return {
    ...
    extractedText: text,
    summary: buildSummary(title, module.title, templateKey),
...
function buildSummary(title, moduleTitle, templateKey) {
  if (templateKey === "nr35") {

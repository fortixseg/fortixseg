...
export async function loadDatabaseState({ defaultCourseCatalog, defaultAppData }) {
  if (!DATABASE_ENABLED) return null;
  await ensureSchema();

  const [courseCatalog, appData] = await Promise.all([
...
export async function saveDatabaseState({ courseCatalog, appData }) {
  if (!DATABASE_ENABLED) return;
  await ensureSchema();
  await Promise.all([
